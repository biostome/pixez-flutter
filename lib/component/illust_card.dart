...
class _IllustCardState extends State<IllustCard> {
  ...
  Widget _buildPic(String tag, bool tooLong) {
    final url = switch (userSetting.feedPreviewQuality) {
      ...
      2 =>
        store.illusts!.metaSinglePage?.originalImageUrl ??
            store.illusts!.metaPages[0].imageUrls!.original,
      _ => store.illusts!.imageUrls.medium,
    };
    final quality = IllustQualityExtension.fromValue(
      userSetting.feedPreviewQuality,
    );
    return tooLong
        ? NullHero(
            tag: tag,
        ...
        : NullHero(
            ...
            child: PixivImage(
              url,
              fit: BoxFit.fitWidth,
              cacheHeaderData: Pivi18CacheHeaderData(key: tag, quality: quality),
            ),
          );
  }
  ...
  Widget _buildBottom(BuildContext context) {
    return Row(
      children: <Widget>[
        ...
        IconButton(
          icon: Icon(
            store.state == 0 ? Icons.favorite_border : Icons.favorite,
            color: store.state == 0 
          ...
          onLongPress: () async {
            ...
            await Navigator.of(context).push(
              MaterialPageRoute(
                builder: (context) => TagForIllustPage(
                  id: store.illusts!.id,
                ),
              ),
            );
