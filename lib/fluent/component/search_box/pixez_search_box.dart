...
class Pivi18SearchBox extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _Pivi18SearchBoxState();
}

class _Pivi18SearchBoxState extends State<StatefulWidget> {
  final GlobalKey<AutoSuggestBoxState<_Pivi18SearchBoxItem>> _key =
      GlobalKey<AutoSuggestBoxState<_Pivi18SearchBoxItem>>();
  ...
  final List<_Pivi18SearchItem> _items = [];
  bool _isLoading = false;
  _Pivi18SearchItem? _cache;
  ...
  Widget build(BuildContext context) {
    return AutoSuggestBox<_Pivi18SearchBoxItem>(
      key: _key,
      items: _items,
  ...
  Future<void> _updateSuggestList() async {
    ...
    if (text.isEmpty) {
      ...
      _items.add(_Pivi18SearchItem.cleanHistory(context));
      _items.addAll(tagHistoryStore.tags
          .map((i) =>
              _Pivi18SearchItem.tagsPersist(context, i, _updateSuggestList))
          .toList());
      ...
      _items.addAll(_trendTagsStore.trendTags
          .map((i) => _Pivi18SearchItem.trendTags(context, i))
          .toList());
      _notifyFinished();
    } else {
      _items.clear();
      ...
      if (id != null) {
        _items.add(_Pivi18SearchItem.illustId(context, id));
        _items.add(_Pivi18SearchItem.painterId(context, id));
        _items.add(_Pivi18SearchItem.pixivisionId(context, id));
        _notifyFinished();
      }
      ...
      _delay = Timer(const Duration(seconds: 1), () async {
        ...
        _items.addAll(
            tags.map((i) => _Pivi18SearchItem.tags(context, i, text)).toList());
        _notifyFinished();
      });
    }
  }
  ...
  void _onAutoSuggestBoxChanged(String text, TextChangedReason reason) {
    ...
    if (_cache?.value?.type != _Pivi18SearchBoxItemType.tag) return;
  ...
  void _onAutoSuggestBoxSelected(
      AutoSuggestBoxItem<_Pivi18SearchBoxItem> value) {
    _cache = value as _Pivi18SearchItem?;
    final item = value.value;
    if (item == null) return;
    if (item.type == _Pivi18SearchBoxItemType.tag) return;

    FocusScope.of(context).unfocus();
    switch (item.type) {
      case _Pivi18SearchBoxItemType.history:
        Leader.push(
          context,
          ResultPage(
            word: item.word!,
      ...
      case _Pivi18SearchBoxItemType.illustId:
        Leader.push(
          context,
      ...
      case _Pivi18SearchBoxItemType.painterId:
        Leader.push(
          context,
      ...
      case _Pivi18SearchBoxItemType.pixivisionId:
        Leader.push(
          context,
          SoupPage(
      ...
      case _Pivi18SearchBoxItemType.cleanHistory:
        showDialog(
          context: context,
          builder: (context) {
